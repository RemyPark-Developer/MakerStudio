...
export default async function ExamplesPage() {
  ...
  const stats = await getContentStats(examples.map((e) => e.id));
        ...
        return (
          <Link key={ex.id} href={`/examples/${ex.id}`} className="examplelink">
            <div className="card">
              ...
              <span className="pill">👁 {s.viewCount.toLocaleString()}</span>
              {s.avgRating !== null && (
                <span className="pill">⭐ {s.avgRating.toFixed(1)} ({s.ratingCount})</span>
              )}

...
**버전**: v1.2 · **작성일**: 2026-08-13 · **최종 수정**: 2026-08-23(5단계 "결제 재시도" 정리 완료 반영)

이전 수정(v1.1, 2026-08-22): 1~7단계 완료 현황을 실제 코드와 대조해 정정, 8단계 배포/모니터링은 전부 미착수 확인
...
> "재시도"만 정리 필요로 확인(2026-08-23 완료 — 아래 5단계 항목 참고), 6단계 완료, 7단계는
> 위시리스트만 미구현). **8단계
...
- 구독 해지 — 완료. **결제 실패/재시도 — 2026-08-23 정리 완료**: 전용 API는 만들지 않기로 결정(일회성 브라우저 결제 구조라 서버가 대신 재결제할 방법이 없음). 즉시 실패는 체크아웃 화면이 이미 처리(에러 표시 후 버튼 재사용 가능), 비동기 실패(웹훅 `Transaction.Failed`)는 `notifications`(`payment_failed`/`payment_activation_failed`)의 `action_url` 재시도 링크를 `/mypage/billing` 상단 배너로 노출하도록 구현(`API_Spec.md` §3 참고)

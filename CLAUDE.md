...
- `supabase/migrations/0001_init.sql`, `0002_tutor_usage_increment.sql`, `0003_auto_create_profile.sql`, `0004_grant_service_role.sql` — DB 스키마 전체 + AI튜터 사용량 원자적 증가 함수 + 회원가입 시 profiles 자동생성 트리거 + service_role 권한 부여. 전부 로컬 Postgres로 실제 실행·검증 완료. **Supabase에 처음 적용할 때 0001→0002→0003→0004 순서대로 SQL Editor에서 실행할 것.**

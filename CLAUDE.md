...
- **소셜 로그인 — 구글(2026-08-22)** — `lib/supabase/browser.ts`(브라우저 전용 anon-key 클라이언트, `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY` 신규 사용), `app/SocialLoginButtons.tsx`, `app/auth/callback/page.tsx`. `POST /api/identity/signup/social` 같은 커스텀 서버 엔드포인트는 안 만듦 — 브라우저가 `supabase.auth.signInWithOAuth()`로 Supabase와 직접 통신하고, 콜백 페이지가 그 세션의 토큰을 기존 `ms_access_token`/`ms_refresh_token`(localStorage) 체계로 그대로 옮겨서 `authedFetch` 등 기존 인프라를 그대로 재사용한다. 온보딩(신규 사용자 닉네임 입력)도 새 서버 로직 없이 기존 `GET/PATCH /api/identity/me`(`needsNickname` 판단)를 그대로 재사용. role은 무조건 `student_teen`(§3.3, 소셜 로그인은 만 14세 이상으로 간주). **카카오 버튼도 2026-08-22에 추가됨**(`startOAuth("google" | "kakao")`, 구글과 동일한 리다이렉트 방식 — Supabase가 카카오에 토큰 방식 `signInWithIdToken`을 지원하지 않아 처음부터 이렇게 통일 설계함). Supabase 프로바이더 등록까지는 완료해서 인가 요청이 실제 카카오 로그인 화면까지는 도달하는 것을 확인했지만, **2026-08-23 기준 카카오 로그인은 보류 상태다** — `account_email` 스코프가 카카오 비즈니스 정보 심사 승인 후에만 열리는 구조라 심사 제출 전(실서비스 URL 미확보)이라 정식 진행이 막혀있다. 코드/설정은 지금 그대로 유지(이메일 스코프 포함, null 이메일 예외처리 없음이 원래 설계이고 그대로 둘 것 — 이메일 없이 허용하는 임시 우회는 도입하지 않는다). **당분간 로그인 관련 개발·검증은 구글 계정 기준으로만 진행하고, 심사 통과 후 카카오 이메일 포함 정식 클릭 테스트를 다시 진행한다.** 상세: `docs/MakerStudio_Auth_Flow_v1.2.md` §2.1/2.2.
...
- **콘텐츠 조회수·평점·관리자 통계(2026-08-22, `0037`)** — `content_ratings` 테이블(사용자당 콘텐츠 1개, 1~5 정수, upsert) 하나만 신규 생성하고 조회수·최근 활동자 수는 새 테이블 없이 `learning_progress` 집계 뷰(`content_view_counts`=고유 응시자 수, `content_recent_activity`=최근 5분 근사치)로 구현. `content_rating_summary` 뷰가 평점 집계를 공개하는데, **개별 평점 행 자체는 본인/admin만 조회 가능**(RLS로 분리) — 뷰 3개는 익명 집계라 anon/authenticated GRANT해도 안전하다는 게 판단 근거(GRANT는 이 마이그레이션도 위 절대 규칙대로 정책과 세트로 넣음). `app/api/learning/rating`(GET/POST, `learning` 도메인)·`lib/content/contentStats.ts`(공개 카탈로그·상세용)·`lib/content/adminContentStats.ts`+`app/api/content/admin/stats`(관리자 전용, 정적 예제+`content_modules` 전 버전/상태 병합)·`app/examples/[id]/RatingWidget.tsx`·`/admin/dashboard`에 매출/콘텐츠 탭 분리 추가. **통계는 Premium 게이팅과 무관하게 항상 노출**된다(`gateExample()`이 잠근 뒤에도 `viewCount`/`avgRating`은 응답에 포함 — 절대 원칙 2번의 "code/explain/quiz"엔 통계 수치가 해당 안 됨). 임시 테스트 계정(student_teen+admin, service_role로 생성 후 삭제) 만들어 별점 등록/수정/범위 밖 값 거부/비로그인 401/admin 아닌 계정의 관리자 통계 401/admin 통계 정상 응답까지 실DB로 검증 완료, 삭제 시 `content_ratings`가 FK cascade로 같이 정리되는 것도 확인.
...
- 소셜 로그인 — 카카오 — 구글은 2026-08-22 완료. 카카오는 버튼·Supabase 프로바이더 등록까지는 끝났지만(인가 요청이 실제 카카오 로그인 화면까지 도달하는 것 확인됨), **`account_email` 스코프가 카카오 비즈니스 정보 심사 승인 후에만 열리는 구조라 2026-08-23 기준 보류 상태**(심사 제출 전, 실서비스 URL 미확보). 코드/설정은 지금 그대로 유지 — 이메일 없이 허용하는 임시 우회는 도입하지 않는다. 당분간 로그인 관련 개발·검증은 구글 계정 기준으로만 진행. 심사 통과 후 할 일: 카카오 실제 클릭 테스트(닉네임/프로필/이메일 모두 포함) + 신규 유저 온보딩 분기가 카카오/구글 양쪽에서 동일하게 동작하는지 확인.
...
- 해지 후 데이터 자동 파기(cron) — `scripts/purge-expired-data.ts`(수동 실행, 2026-08-22)까지만 준비됨, 인프라 결정되면 그대로 스케줄링에 연결. 개인정보처리방침 전체 문서화(지금은 이 조항 하나만 초안, `MakerStudio_Privacy_Policy_DRAFT_addendum_v0.1.md`) — **법률 검토도 아직 안 됨, 대외 공개 금지.** 해지 시 학습 포트폴리오 PDF 다운로드 — PDF 생성 인프라(라이브러리·폰트) 자체가 이 저장소에 없어서 별도 작업으로 분리(기존 "PDF 워크북/수료증" 항목과 같은 근본 원인).

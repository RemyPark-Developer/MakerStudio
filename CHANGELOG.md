...
- 카카오 로그인은 `account_email` 스코프가 비즈니스 정보 심사 승인 후에만 열리는 구조라,
  현재 심사 제출 전 상태(실서비스 URL 미확보로 보류). 코드/Supabase 설정은 지금 그대로
  유지(이메일 스코프 포함, null 이메일 예외처리 없음 — 이 상태가 원래 설계).
- 구글 로그인으로 개발 계속 진행하고, 심사 통과 후 카카오 로그인 이메일 포함 정식 테스트 예정.

## v2.35 — 2026-08-22

**요약**: 카카오 소셜 로그인 버튼 추가(`app/SocialLoginButtons.tsx`) — 구글 버튼과 동일한
리다이렉트 방식으로 통일(`v2.31`에서 설계된 구조 그대로 재사용, 코드 변경은 이 버튼 추가뿐).

- 변경: `startOAuth`의 파라미터 타입을 `"google"` → `"google" | "kakao"`로 확장, "카카오로
  시작하기" 버튼 추가. `app/auth/callback/page.tsx`는 provider에 무관하게 세션만 브리징하므로
  수정 불필요
- **⚠️ 대표님이 Supabase 대시보드에 카카오 프로바이더(Client ID/Secret)를 아직 등록 안 함** —
  등록 전까지 버튼을 눌러도 에러 처리 문구만 뜬다. 등록 완료 후 실제 로그인 동작 재검증 필요
  (코드 변경은 불필요할 것으로 예상)
